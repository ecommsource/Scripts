...
project_menu() {
  ...
  while [ "$DONE2" = "no" ]
    do
      ...
      echo "  E) Edit This Project's Settings."
...
edit_project() {
  ...
  TEMP6=
  echo "DEVORG is the Development ScratchOrg Alias."
  echo "DEVORG=$DEVORG"
  echo "Enter DEVORG ->\c"; read TEMP6
  ...
  if [ "$TEMP6" != "" ] ; then DEVORG=$TEMP6 ; fi
  eval "PROJECT$INDEX=$PROJECT"
  eval "FOLDER$INDEX=$FOLDER"
  eval "USER$INDEX=$USER"
  eval "PASS$INDEX=$PASS"
  eval "GITPATH$INDEX=$GITPATH"
  eval "DEVORG$INDEX=$DEVORG"
  echo "PROJECT=$PROJECT"
  echo "FOLDER=$FOLDER"
  echo "USER=$USER"
  echo "PASS=$PASS"
...
save_ini() {
  ...
  for i in `seq 1 $MAX_PROJECTS`; do
    ...
    VAL=${!VAR}
    if [ "$VAL" != "" ]  ; then 
      if [ "$DEBUG" = "1" ] ; then echo "Saving Project $j ..." ; fi
      echo "#= Project #$j -------------------" >> $INI_FILE
      if [ "$DEBUG" = "1" ] ; then echo "  Project Name: $VAL" ; fi
      echo "PROJECT$j=$VAL" >> $INI_FILE
      VAR="FOLDER$i"; VAL=${!VAR}; echo "FOLDER$j=$VAL" >> $INI_FILE
      if [ "$DEBUG" = "1" ] ; then echo "  Folder: $VAL" ; fi
      VAR="USER$i"; VAL=${!VAR}; echo "USER$j=$VAL" >> $INI_FILE
      if [ "$DEBUG" = "1" ] ; then echo "  Username: $VAL" ; fi
      VAR="PASS$i"; VAL=${!VAR}; echo "PASS$j=$VAL" >> $INI_FILE
      if [ "$DEBUG" = "1" ] ; then echo "  Password: $VAL" ; fi
      VAR="GITPATH$i"; VAL=${!VAR}; echo "GITPATH$j=$VAL" >> $INI_FILE
      if [ "$DEBUG" = "1" ] ; then echo "  GIT Path: $VAL" ; fi
      VAR="DEVORG$i"; VAL=${!VAR}; echo "DEVORG$j=$VAL" >> $INI_FILE 
      if [ "$DEBUG" = "1" ] ; then echo "  Dev Org: $VAL" ;pause; fi
      j=$((j + 1))  
    fi
  done 
...
sfdx_menu() {
  ...
  while [ "$DONE4" == "no" ]
    ...
    echo "  Curent Project: ${BOLD}$PROJECT${NORM} - DevOrg: ${BOLD}$DEVORG${NORM}"
    echo "+ -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- +"
    ...
    echo "  R) Reset Development Org."

    echo "  Q or X) Quit (Exit SFDX Menu)"
    echo "Please Select ->\c"; read OPT4
    case $OPT4 in
        o|O) echo "Open Default Development Org '$DEVORG' (Y/N) ? \c";read YN6
             if [ "$YN6" == "y" ] ; then
	       sfdx force:org:open -u "$DEVORG" ; 
             else
               echo "Loading List of Scratch Orgs ..."
               sfdx force:org:list
               ORGNAME=
	         ...
	         sfdx force:org:open -u "$ORGNAME" ; 
                 echo "Is this your Default Development Org for this project (Y/N) ?\c"
                 read YN5
                 if [ "$YN5" == "y" ] ; then
                   set DEVORG=$ORGNAME
                   set DEVORG$INDEX=$ORGNAME
                   save_ini
                   fi
                 fi ### end of if $ORGNAME not blank
               fi ### end of if/else $YN6
             pause ;;
        ...
        c|C) echo "Please Enter a Unique Name for this scratch org ->\c";read ORGNAME
             ...
             echo "Is this your Main Development Org for this project (Y/N) ? \c"
             read YN4D
             if [ "$YN4D" == "y" ] ; then
               DEVORG=$ORGNAME
               eval "DEVORG$INDEX=$DEVORG"
        ...
        a|A) echo "Add Prerequisite Packages to deploy when creating Scratch Orgs:"
             ...
             ;;
        r|R) echo "Reset (Choose) Default Development Scratch Org for this project:"
             echo "DEVORG Currently set to $DEVORG"
             echo "List Current Scratch Orgs (Y/N) ? \c";read YN5;
             if [ "$YN5" == "y" ] ; then
               sfdx force:org:list
               fi
             TEMP1=
             echo "Enter the Default Development Org for this project ->\c"
             read TEMP1
             if [ "$TEMP1" != "" ] ; then
               echo "Saving to index $INDEX ..."
               DEVORG=$TEMP1
               eval "DEVORG$INDEX=$DEVORG"
               save_ini
             else
               echo "DEVORG was not changed for this project."
               fi
             pause
             ;;
      	q|Q|x|X) DONE4="yes";;
    ...
    if [ -f $INI_FILE ]
      ...
      for i in `seq 1 $MAX_PROJECTS`; do 
        ...
        if [ "$TEMP1" != "" ]
          ...
          TEMP6=`grep DEVORG$i $INI_FILE | cut -f 2 -d =`
          declare DEVORG$i=${TEMP6//[$'\t\n\r']}
